...
def read_tissue_metadata(f, annotation):
    ...
    for c in tissue_metadata[annotation].unique():
        c_compartment = tissue_metadata[tissue_metadata[annotation] == c][
            "compartment"
        ].unique()
        c_compartment = [i.capitalize() for i in c_compartment]

        c_tissue = tissue_metadata[tissue_metadata[annotation] == c][
            "organ_tissue"
        ].unique()
    ...
    return (
        ...
        tissue_metadata["compartment"].unique(),
        celltype2tissue,
        tissue_metadata["organ_tissue"].unique(),
    )


def filter_model_data(df, test_only):
    if test_only:
        df = df[df["type"] == "test"]  # Keep only test for evaluation
    else:
        df = df[df["type"] != "train"]  # Remove train for evaluation
    df = df[df["y"] >= 0]
    return df
...
def map_to_compartment(metric, celltype2compartment):
    ...
    metric_df = pd.DataFrame.from_dict(
        {"celltype": list(metric.keys()), "metric": list(metric.values())}
    )
    celltype2compartment_df = pd.DataFrame.from_dict(
        {
            "celltype": list(celltype2compartment.keys()),
            "compartment": list(celltype2compartment.values()),
        }
    )

    metric_by_compartment = metric_df.merge(celltype2compartment_df)
    assert len(metric_by_compartment) == len(metric), (
        len(metric_by_compartment),
        len(metric),
    )
...
def get_celltype2tissue(metagraph, bto_names):
    ...
    for n in metagraph.nodes():
        ...
        if len(tissue_neighbors) == 1:
            celltype2tissue[n] = bto_names[tissue_neighbors[0]]
        else:
            celltype2tissue[n] = ";".join([bto_names[t] for t in tissue_neighbors])
...
def check_available_celltypes(celltypes, model_outputs_df):
    ...
    assert len(set(model_celltypes).intersection(set(celltypes))) == len(
        set(model_celltypes)
    )
