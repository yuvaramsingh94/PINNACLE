...
def plot_PINNACLE_embeddings(
    ...
    plot_protein_umap(
        protein_labels,
        color_map,
        output_dir + "umap.all=%s_mindist=%s" % (n_neighbors, min_dist),
    )
    ...
    if do_plot_metagraph:
        ...
        plot_metagraph_umap(
            ...
            protein_labels,
            color_map,
            output_dir + "umap.ccibto=%s_mindist=%s" % (n_neighbors, min_dist),
        )
...
def plot_emb(
    ...
    color_map = {
        n: c
        for n, c in zip(
            metagraph.nodes, sns.color_palette("husl", len(metagraph.nodes))
        )
    }  # TODO
    if do_plot_metagraph:
    ...
    if do_sweep:
        for n_neighbors in umap_param["n_neighbors"]:
            for min_dist in umap_param["min_dist"]:
                outfile = output_dir + (
                    "embedding_nneighbors=%s_mindist=%s.npy" % (n_neighbors, min_dist)
                )
    ...
    # Fit and plot desired UMAP parameters
        ...
        outfile = output_dir + (
            "embedding_nneighbors=%s_mindist=%s.npy"
            % (umap_param["n_neighbors"], umap_param["min_dist"])
        )
        if not os.path.exists(outfile):
            print("Fit UMAP...")
            mapping, embedding = fit_umap(
                embed, umap_param["n_neighbors"], umap_param["min_dist"]
            )
            np.save(outfile, embedding)
        else:
            embedding = np.load(outfile)
...
def plot_protein_umap(labels, color_map, output):
    ...
    for celltype in [
        ...
        print(celltype)
        celltype_colormap = {
            k: "#a3a2a2" if k != celltype else v for k, v in color_map.items()
        }
        fig, ax = plt.subplots(figsize=FIGSIZE)
        sns.scatterplot(
            data=df[df["Cell Type"] != celltype],
            x="x",
            y="y",
            hue="Cell Type",
            marker=".",
...
def plot_metagraph_umap(labels, protein_labels, color_map, output):
    ...
    fig, ax = plt.subplots(figsize=FIGSIZE)
    sns.scatterplot(
        data=pd.DataFrame(protein_labels),
        x="x",
        y="y",
        marker=".",
        color="#a3a2a2",
    ...
    sns.scatterplot(
        data=df[~df["Cell Type"].str.contains("BTO")],
        x="x",
        y="y",
        marker="^",
        color="seagreen",
...
def main():
    ...
    ppi_x, mg_x, labels_dict = read_embed(
        input_f + "pinnacle_protein_embed.pth",
        input_f + "pinnacle_mg_embed.pth",
        input_f + "pinnacle_labels_dict.txt",
    )
    ...
    sanity_idx = [
        i for i, l in enumerate(labels_dict["Cell Type"]) if "Sanity" not in l
    ]
    ...
    if args.do_sweep:
        umap_param = {
            ...
            "min_dist": [0.1, 0.15, 0.2, 0.25, 0.5, 0.75, 0.8, 0.9],
        }
    else:
