...
def split_data(num_y):
    ...
    val_idx = split_idx[
        int(len(split_idx) * 0.8) : int(len(split_idx) * 0.9)
    ]  # Val mask
...
def read_ppi(ppi_dir):
    ...
    for f in glob.glob(
        ppi_dir + "*edgelist.txt"
    ):  # Expected format of filename: <PPI_DIR>/<CONTEXT>.<suffix>
        ...
        ppi_train[context], ppi_val[context], ppi_test[context] = split_data(
            len(ppi_layers[context].edges)
        )
...
def read_global_ppi(f):
    ...
    edges = [
        (s, t)
        for s, t in zip(graph_df["protein1"].tolist(), graph_df["protein2"].tolist())
    ]
...
def read_data(G_f, ppi_dir, mg_f, feat_mat_dim):
    ...
    # Read metagraph
    metagraph = nx.read_edgelist(
        mg_f, data=False, delimiter="\t", create_using=nx.DiGraph
    )
    ...
    mg_mapping.update(
        {
            n: i + len(ppi_layers)
            for i, n in enumerate(sorted([n for n in metagraph.nodes if "BTO" in n]))
        }
    )
    assert len(mg_mapping) == len(metagraph.nodes), set(metagraph.nodes).difference(
        set(list(mg_mapping.keys()))
    )
    print(mg_mapping)

    # Set up Data object
    mg_nodetype = [
        0 if "BTO" in n else 1 for n in mg_mapping
    ]  # Tissue nodes = 0, Cell-type nodes = 1, protein nodes = 2
    mg_edgetype = []
    for edges in metagraph.edges:
        if "BTO" in edges[0] and "BTO" in edges[1]:
            mg_edgetype.append(0)  # tissue-tissue edge
        elif "BTO" in edges[0] and "BTO" not in edges[1]:
            mg_edgetype.append(1)  # tissue-cell edge
        elif "BTO" not in edges[0] and "BTO" in edges[1]:
            mg_edgetype.append(2)  # cell-tissue edge
        elif "BTO" not in edges[0] and "BTO" not in edges[1]:
            mg_edgetype.append(3)  # cell-cell edge
        else:
            print(edges)
            raise NotImplementedError
    tissue_neighbors = {
        mg_mapping[t]: [mg_mapping[n] for n in metagraph.to_undirected().neighbors(t)]
        for t in metagraph.to_undirected().nodes
        if "BTO" in t
    }
    metagraph = nx.relabel_nodes(metagraph, mg_mapping)
    mg_mask = torch.ones(
        len(metagraph.edges), dtype=torch.bool
    )  # Pass in all meta graph edges during training, validation, and test
    mg_data = create_data(
        metagraph, mg_mask, mg_mask, mg_mask, mg_nodetype, mg_edgetype, mg_feat_mat
    )

    # Set up PPI Data objects
    orig_ppi_layers_remapped = {
        mg_mapping[k]: v for k, v in orig_ppi_layers.items() if k in mg_mapping
    }
...
def get_centerloss_labels(args, celltype_map, ppi_layers):
    ...
    center_loss_idx = random.sample(
        range(len(center_loss_labels)), len(center_loss_labels)
    )
    train_mask = center_loss_idx[: int(0.8 * len(center_loss_idx))]
    val_mask = center_loss_idx[
        len(train_mask) : len(train_mask) + int(0.1 * len(center_loss_idx))
    ]
