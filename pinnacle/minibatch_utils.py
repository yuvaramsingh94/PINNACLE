...
def iterate_train_batch(
    ...
    ppi_preds_all = {}
    ppi_data_y = {
        key: {"y": torch.tensor([]), "total_edge_type": torch.tensor([])}
        for key in ppi_x_ori.keys()
    }
    ppi_x_out = {
        key: torch.zeros((x.shape[0], model.output)) for key, x in ppi_x_ori.items()
    }
    count = 0
    ...
    for packed_batch in zip(*ppi_train_loader_dict.values()):
        ...
        ppi_data_batch, ppi_x, ppi_node_ind_batch, ppi_metapaths_batch, _ = (
            train_batch2dict(
                packed_batch,
                mg_x_ori,
                ppi_metapaths_ori,
                list(ppi_train_loader_dict.keys()),
                device,
            )
        )
        batch_size = sum(
            [data["y"].shape[0] for data in ppi_data_batch.values()]
        )  # Number of all samples across all cell types
        ...
        mg_pred = el_dot(
            mg_x,
            mg_data_train["total_edge_index"],
            model.mg_relw[mg_data_train["total_edge_type"]],
        )

        # Compute predictions for PPI layers
        ppi_preds = dict()
        for celltype, x in ppi_x.items():
            ppi_preds[celltype] = el_dot(
                x, ppi_data_batch[celltype]["total_edge_index"], []
            )
            ppi_preds_all[celltype] = torch.cat(
                [
                    ppi_preds_all.setdefault(celltype, torch.tensor([])),
                    ppi_preds[celltype].detach().cpu(),
                ]
            )
            ppi_data_y[celltype]["y"] = torch.cat(
                [
                    ppi_data_y[celltype]["y"],
                    ppi_data_batch[celltype]["y"].detach().cpu(),
                ]
            )
            ppi_data_y[celltype]["total_edge_type"] = torch.cat(
                [
                    ppi_data_y[celltype]["total_edge_type"],
                    ppi_data_batch[celltype]["total_edge_type"].detach().cpu(),
                ]
            )
            ppi_x_out[celltype][ppi_node_ind_batch[celltype]] = x.detach().cpu()

        # Compute train loss
        ppi_loss, mg_loss = calc_link_pred_loss(
            mg_pred, mg_data_train, ppi_preds, ppi_data_batch, hparams["loss_type"]
        )
        ...
        # Protein labels
        center_loss_labels = torch.cat(
            [(torch.ones(x.shape[0]) * key).to(torch.long) for key, x in ppi_x.items()]
        )  # Build center loss labels based on batched nodes to ensure consistency with embedding labels

        # Train mask
        train_mask = construct_batch_center_loss_mask(
            mask_train_ori, ppi_node_ind_batch, ppi_x_ori
        )

        # Center loss
        cent_loss = calc_center_loss(
            center_loss, embed, centers, center_loss_labels, train_mask
        )
    ...
    for packed_batch in zip(*ppi_loader_dict.values()):
        ...
        if count == 1:
            mg_pred = el_dot(
                mg_x.to(device),
                mg_data["total_edge_index"],
                model.mg_relw[mg_data["total_edge_type"]],
            )
        ...
        for celltype, x in ppi_x.items():
            ppi_preds[celltype] = el_dot(
                x.to(device),
                ppi_data_batch[celltype]["total_edge_index"].to(device),
                [],
            )
            ppi_preds_all[celltype] = torch.cat(
                [
...
def construct_batch_center_loss_mask(
    original_mask: list, ppi_node_ind_batch: dict, ppi_x_ori: dict
) -> list:
    """
    ...
    ppi_node_ind_batch_concat = (
        torch.cat(
            [
                value + ppi_cum_size[i]
                for i, value in enumerate(ppi_node_ind_batch.values())
            ]
        )
        .detach()
        .cpu()
    ...
    train_mask_batch = [
        i for i, ind in enumerate(ppi_node_ind_batch_concat) if ind in original_mask_set
    ]
...
def train_batch2dict(
    ...
    for ind, batch in enumerate(packed_batch):
        ...
        # Metapath adjs
        ppi_metapaths_batch = construct_metapath(
            ppi_metapaths,
            batch.edge_index[:, batch.y.type(torch.bool)],
            batch.edge_attr[batch.y.type(torch.bool)],
            batch.x.shape[0],
        )
...
def generate_batch(
    ...
    for key, data in data_dict.items():
        ...
        neg_edge_index, neg_edge_type = negative_sampler(
            pos_edge_index, edge_type, edge_attr_dict
        )
        ...
        # Metapath adjs
        metapath_adjs_dict[key] = construct_metapath(
            metapaths, pos_edge_index, edge_type, data.x.size(0)
        )
        ...
        if ppi:
            data = Data(
                x=data.x, edge_index=total_edge_index, edge_attr=total_edge_type, y=y
            )
            ...
            if loader_type == "neighbor":
                ...
                loader = GraphSAINTEdgeSampler(
                    data, batch_size=batch_size, num_steps=16
                )
            else:
...
def negative_sampler(pos_edge_index, edge_type, edge_attr_dict):
    ...
    for attr, idx in edge_attr_dict.items():
        ...
        neg_source, neg_target, neg_rand = structured_negative_sampling(
            pos_rel_edge_index
        )
