...
class PCTConv(nn.Module):
    def __init__(
        ...
        self.mg_conv_out = GATv2Conv(
            out_channels * node_heads, out_channels, node_heads
        )

        # Semantic attention (shared across networks)
        self.W = nn.Parameter(
            torch.Tensor(1, 1, out_channels * node_heads, sem_att_channels)
        )
        ...
        self.pc_W = nn.Parameter(
            torch.Tensor(1, 1, out_channels * node_heads, pc_att_channels)
        )
    ...
    def _per_data_forward(self, x, edgetypes, node_conv):
        ...
        out = [
            node_conv(x, edgetype) for edgetype in edgetypes if edgetype.shape[1] > 0
        ]
    ...
    def forward(
        ...
        for (
            celltype,
            x,
        ) in ppi_x.items():  # Iterate through cell-type specific PPI layers
            if len(ppi_metapaths[celltype]) == 0:
                ppi_x[celltype] = []
            else:
                ppi_x[celltype] = self._per_data_forward(
                    x, ppi_metapaths[celltype], self.ppi_w[celltype]
                )
            ...
            if init_cci:  # Initialize CCI embeddings using PPI embeddings
                weighted_x = torch.sum(
                    ppi_x[celltype] * self.ppi_attn[celltype].unsqueeze(-1), dim=0
                )
                mg_x_list.append(weighted_x)
            else:  # Update CCI embeddings
                mg_x[celltype, :] += torch.sum(
                    ppi_x[celltype] * self.ppi_attn[celltype].unsqueeze(-1), dim=0
                )
        ...
        for i in range(
            self.tissue_update
        ):  # Initialize tissue embeddings in a more meaningful way
...
class PPIConv(nn.Module):
    ...
    def _per_data_forward(self, x, metapaths, node_conv):
        ...
        # Calculate node-level attention representations
        out = [
            node_conv(x, metapath) for metapath in metapaths if metapath.shape[1] > 0
        ]
        out = torch.stack(out, dim=1).to(x.device)
    ...
    def forward(self, ppi_x, ppi_metapaths, mg_x, ppi_attn):
        ...
        for (
            ...
            gamma = ppi_attn[
                celltype
            ]  # (n,) where n = number of proteins in the cell type (in the batch)
            ppi_x[celltype] += mg_x[celltype, :].repeat(
                len(gamma), 1
            ) * gamma.unsqueeze(-1)
