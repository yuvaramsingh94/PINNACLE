...
def metrics_per_rel(
    ...
    if len(mg_pred) > 0:
        for attr, idx in edge_attr_dict.items():
            ...
            wandb.log(
                {
                    "%s_%s_roc" % (attr, split): roc_per_rel,
                    "%s_%s_ap" % (attr, split): ap_per_rel,
                    "%s_%s_acc" % (attr, split): acc_per_rel,
                    "%s_%s_f1" % (attr, split): f1_per_rel,
                }
            )
    ...
    for celltype, ppi in ppi_preds.items():
        for attr, idx in edge_attr_dict.items():
            ...
            log_f.write(
                "ROC for edge type {} in celltype {}: {:.5f}\n".format(
                    attr, celltype_map[celltype], roc_per_rel
                )
            )
            log_f.write(
                "AP for edge type {} in celltype {}: {:.5f}\n".format(
                    attr, celltype_map[celltype], ap_per_rel
                )
            )
            log_f.write(
                "ACC for edge type {} in celltype {}: {:.5f}\n".format(
                    attr, celltype_map[celltype], acc_per_rel
                )
            )
            log_f.write(
                "F1 for edge type {} in celltype {}: {:.5f}\n".format(
                    attr, celltype_map[celltype], f1_per_rel
                )
            )
            wandb.log(
                {
                    "%s_%s_%s_roc" % (celltype_map[celltype], attr, split): roc_per_rel,
                    "%s_%s_%s_ap" % (celltype_map[celltype], attr, split): ap_per_rel,
                    "%s_%s_%s_acc" % (celltype_map[celltype], attr, split): acc_per_rel,
                    "%s_%s_%s_f1" % (celltype_map[celltype], attr, split): f1_per_rel,
                }
            )
...
def construct_metapath(metapaths, edge_index, edge_type, num_nodes):
    ...
    mp_adjs = [
        torch.stack([mp_adj.storage.row(), mp_adj.storage.col()]) for mp_adj in mp_adjs
    ]
    return mp_adjs
...
def plot_emb(
    ...
    celltype_map = {v: k for k, v in celltype_map.items()}
    embed, labels_df, mg_labels = combine_embed(
        best_ppi_x, best_mg_x, celltype_map, ppi_layers, metagraph, finetune_labels
    )
...
def combine_embed(ppi_embed, mg_embed, key, ppi_layers, metagraph, finetune_labels):
    ...
    if len(mg_embed) > 0:
        ...
        labels_df["Cell Type"] = [
            "CCI_" + v if "BTO" not in v else v for k, v in key.items()
        ]
    ...
    for cluster, ppi in ppi_layers.items():
        if cluster in key.values():
            protein_counts += list(ppi)
    ...
    for celltype, x in ppi_embed.items():
        ...
        degrees = [
            ppi_layers[key[celltype]].degree[n] for n in ppi_layers[key[celltype]].nodes
        ]
...
def plot_umap(labels, wandb, wb_title, color_category="default", finetune_labels=[]):
    ...
    fig = px.scatter(
        df, x="x", y="y", color="Cell Type", size="Degree", hover_data=hover_keys
    )
