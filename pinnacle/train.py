...
def train(epoch, model, optimizer, center_loss):
    ...
    # Run batch training
    _, _, mg_pred, ppi_preds_all, ppi_data_train_y, loss = mb_utils.iterate_train_batch(
        ppi_train_loader_dict,
        ppi_x_ori,
        ppi_metapaths,
        mg_x_ori,
        mg_metapaths_train,
    ...
    # Training metrics
    roc_score, ap_score, train_acc, train_f1 = utils.calc_metrics(
    ...
    ppi_x, _, mg_pred, ppi_preds_all, ppi_data_val_y = mb_utils.iterate_predict_batch(
        ...
        hparams,
        device,
    )  # Using train metapaths.

    # Validation metrics
    roc_score, ap_score, val_acc, val_f1 = utils.calc_metrics(
        mg_pred, mg_data_val, ppi_preds_all, ppi_data_val_y
    )
    print(
        "Validation Metrics:",
        "ROC",
    ...
    wandb.log(
        {
            "total_loss": loss,
            "total_val_roc": roc_score,
            "total_val_ap": ap_score,
            "total_val_acc": val_acc,
            "total_val_f1": val_f1,
            "total_val_calinski_harabasz_score": calinski_harabasz,
            "total_val_davies_bouldin_score": davies_bouldin,
        }
    )
...
def test(model, ppi_metapaths_test, mg_metapaths_test):
    ...
    roc_score, ap_score, test_acc, test_f1 = utils.calc_metrics(
...
def main():
    ...
    if args.resume_run != "":
        ...
        model = mdl.Pinnacle(
            ...
            ppi_data,
            hparams["n_heads"],
            hparams["pc_att_channels"],
            hparams["dropout"],
        ).to(device)
        params = list(model.parameters())
        optimizer = torch.optim.Adam(
            params, lr=hparams["lr"], weight_decay=hparams["wd"]
        )
    center_loss = CenterLoss(
        num_classes=len(set(center_loss_labels)),
        feat_dim=hparams["output"] * hparams["n_heads"],
    ...
    for epoch in range(args.epochs):
        ppi_metapaths_train, mg_metapaths_train, ppi_metapaths_val, mg_metapaths_val = (
            train(epoch, model, optimizer, center_loss)
        )
    ...
    for key in ppi_metapaths_train.keys():
        ppi_metapaths_test[key] = [
            torch.cat(ppi_metapaths_val[key] + ppi_metapaths_train[key], dim=1)
        ]
    ...
    _, ppi_data_all, ppi_metapaths_adjs, ppi_x = mb_utils.generate_batch(
        ppi_data,
        ppi_metapaths,
        edge_attr_dict,
        "all",
    ...
    _, mg_data_all, mg_metapaths_adjs, mg_x = mb_utils.generate_batch(
        {0: mg_data},
        mg_metapaths,
        edge_attr_dict,
        "all",
    ...
    best_ppi_x, best_mg_x = utils.get_embeddings(
        best_model,
        ppi_x,
        mg_x[0],
        ppi_metapaths_adjs,
        mg_metapaths_adjs[0],
        ppi_data_all,
        mg_data_all[0]["total_edge_index"],
        tissue_neighbors,
    )
    ...
    protein_labels_dict = {
        "Cell Type": protein_celltype_labels,
        "Name": protein_name_labels,
    }
    ...
    mg_labels_dict = {
        "Cell Type": metagraph_celltype_labels,
        "Name": metagraph_name_labels,
    }
