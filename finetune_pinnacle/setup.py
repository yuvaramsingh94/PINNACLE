...
def create_parser():
    ...
    parser.add_argument(
        "--hidden_dim_1", type=int, default=64, help="1st hidden dim size"
    )
    parser.add_argument(
        "--hidden_dim_2", type=int, default=32, help="2nd hidden dim size, discard if 0"
    )
    parser.add_argument(
        "--hidden_dim_3", type=int, default=0, help="3rd hidden dim size, discard if 0"
    )
    ...
    parser.add_argument(
        "--order", type=str, default="nd", help="order of normalization and dropout"
    )
    ...
    parser.add_argument(
        "--positive_proteins_prefix",
        type=str,
        default="../data/therapeutic_target_task/positive_proteins",
    )
    parser.add_argument(
        "--negative_proteins_prefix",
        type=str,
        default="../data/therapeutic_target_task/negative_proteins",
    )
    ...
    parser.add_argument(
        "--random", action="store_true", help="random runs without fixed seeds"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="whether to overwrite the label data or not",
    )
    parser.add_argument("--train_size", type=float, default=0.6)
    parser.add_argument("--val_size", type=float, default=0.2)
    parser.add_argument(
        "--weigh_sample", action="store_true", help="whether to weigh samples or not"
    )  # default = False
    parser.add_argument(
        "--weigh_loss", action="store_true", help="whether to weigh losses or not"
    )  # default = False
    parser.add_argument(
        "--lr_scheduler",
        action="store_true",
        help="whether to use ReduceLROnPlateau learning rate scheduler or not",
    )  # default = False
...
def setup_paths(args):
    ...
    if random_state == None:
        ...
        models_output_dir = (
            args.models_output_dir + args.embed + ("_seed=%s" % str(random_state)) + "/"
        )
        metrics_output_dir = (
            args.metrics_output_dir
            + args.embed
            + ("_seed=%s" % str(random_state))
            + "/"
        )
