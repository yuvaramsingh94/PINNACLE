...
def get_labels_from_evidence(
    celltype_protein_dict: Dict[str, List[str]],
    disease: str,
    evidence_dir: str,
    all_drug_targets_path: str,
    curated_disease_dir: str,
    chembl2db_path: str,
    positive_protein_prefix: str,
    negative_protein_prefix: str,
    raw_data_prefix: str,
    overwrite: bool,
    disease_drug_evidence_prefix="",
    wandb=None,
):
    """
    ...
    dti_tab = pd.read_csv(
        all_drug_targets_path, index_col=0
    )  # approved drug-target table
    ...
    if not overwrite:
        positive_proteins, negative_proteins, all_relevant_proteins = (
            read_labels_from_evidence(
    ...
    if len(positive_proteins) == 0:
        ...
        all_disease = get_disease_descendants(
            disease, source="efo", curated_disease_dir=curated_disease_dir
        )
        ...
        disease_drug_evidence_data = get_all_drug_evidence(
            evidence_files, evidence_dir, all_disease, chembl2db
        )
        ...
        disease_drug_targets = evidence2genename(
            disease_drug_evidence_data, ensg2otgenename
        )

        # Saving disease/drug-target evidence
        if disease_drug_evidence_prefix != "":
            disease_drug_evidence_data["targetId_genename"] = (
                disease_drug_evidence_data["targetId"].map(ensg2otgenename)
            )
            ...
            print(
                Counter(
                    "Phase " + str(a) + "," + str(b)
                    for a, b in zip(
                        disease_drug_evidence_data["clinicalPhase"].tolist(),
                        disease_drug_evidence_data["clinicalStatus"].tolist(),
                    )
                )
            )
            print(disease_drug_evidence_data["drugId"].unique())
            disease_drug_evidence_data.to_csv(
                disease_drug_evidence_prefix + disease + ".csv", index=False, sep="\t"
            )

        # Get positive and negative labels for proteins
        positive_proteins = {
            ct: list(disease_drug_targets.intersection(ppi_proteins))
            for ct, ppi_proteins in celltype_protein_dict.items()
        }  # PPI proteins associated with the disease with drug or clinical candidate > II's evidence
        negative_proteins = {
            ct: list(
                set(ppi_proteins)
                .difference(all_associated_targets)
                .intersection(druggable_targets)
            )
            for ct, ppi_proteins in celltype_protein_dict.items()
        }  # PPI proteins that are not associated with the disease except for text mining, but are still druggable
        ...
        positive_protein_counts_celltype = (
            pd.DataFrame(tmp_positive_proteins)
            .rename(index={ind: ind[:-2] for ind in tmp_positive_proteins.keys()})
            .reset_index()
            .melt(id_vars=["index"])
            .groupby(by=["index", "variable"])
            .aggregate(list)
            .applymap(lambda x: len(np.unique(sum(x, start=[]))))
            .reset_index()
        )
        sns.barplot(
            x="variable", y="value", data=positive_protein_counts_celltype, hue="index"
        )
...
def path_update_fn(args):
    ...
    args.disease_drug_evidence_prefix = os.path.join(
        args.result_dir, args.disease, args.disease_drug_evidence_prefix
    )
    args.positive_proteins_prefix = os.path.join(
        args.result_dir, args.disease, args.positive_proteins_prefix
    )
    args.negative_proteins_prefix = os.path.join(
        args.result_dir, args.disease, args.negative_proteins_prefix
    )
    args.raw_data_prefix = os.path.join(
        args.result_dir, args.disease, args.raw_data_prefix
    )
    return args
...
def main():
    ...
    parser.add_argument(
        ...
        help="Filename (prefix) of cell type PPI.",
    )
    parser.add_argument("--disease", type=str)
    parser.add_argument(
        "--evidence_dir", type=str
    )  # Download OpenTargets ChEMBL evidence
    parser.add_argument(
        "--all_drug_targets_path",
        type=str,
        default="../data/therapeutic_target_task/all_approved.csv",
    )
    parser.add_argument("--curated_disease_dir", type=str)
    parser.add_argument(
        "--chembl2db_path", type=str
    )  # Download mapping from ChEMBL id to DrugBank id from https://ftp.ebi.ac.uk/pub/databases/chembl/UniChem/data/wholeSourceMapping/src_id1/src1src2.txt.gz
    parser.add_argument(
        "--result_dir", type=str, default="data/therapeutic_target_task"
    )
    parser.add_argument(
        "--disease_drug_evidence_prefix", type=str, default="disease_drug_evidence_"
    )
    parser.add_argument(
        "--positive_proteins_prefix", type=str, default="positive_proteins_"
    )
    parser.add_argument(
        "--negative_proteins_prefix", type=str, default="negative_proteins_"
    )
    ...
    positive_proteins, negative_proteins, all_relevant_proteins = (
        get_labels_from_evidence(
            ...
            args.disease,
            args.evidence_dir,
            args.all_drug_targets_path,
            args.curated_disease_dir,
            args.chembl2db_path,
            args.positive_proteins_prefix,
            args.negative_proteins_prefix,
            args.raw_data_prefix,
            overwrite=True,
            disease_drug_evidence_prefix=args.disease_drug_evidence_prefix,
        )
    )
