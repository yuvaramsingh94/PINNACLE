...
def training_and_validation(
    ...
    if not no_val:
        ...
        groups_map_val = np.unique(groups_val, return_inverse=True)[
            0
        ]  # factorize groups_val
    ...
    groups_map_train = np.unique(groups_train, return_inverse=True)[
        0
    ]  # factorize groups_train
    groups_train = np.unique(groups_train, return_inverse=True)[1]

    train_dataset = TensorDataset(
        X_train,
        y_train.unsqueeze(-1),
        torch.from_numpy(cts_train),
    ...
    if not no_val:
        val_dataset = TensorDataset(
            X_val,
            y_val.unsqueeze(-1),
            torch.from_numpy(cts_val),
            torch.from_numpy(groups_val),
        )
    ...
    if weigh_sample:
        ...
        weights = torch.DoubleTensor(
            [1 / class_sample_num[y.int().item()] for y in y_train]
        )
    ...
    if (hparams[norm] == "bn" or hparams[norm] == "ln") and len(
        train_dataset
    ) % batch_size < 3:
        drop_last = True
    ...
    if not no_val:
        ...
        )  # set val batch size to full-batch
    ...
    if hparams[hidden_dim_2] == 0:
        ...
        hidden_dims = [
            hparams[hidden_dim_1],
            hparams[hidden_dim_2],
            hparams[hidden_dim_3],
        ]
    ...
    if weigh_loss:
        pos_weight = torch.Tensor(
            [(y_train.shape[0] - y_train.sum().item()) / y_train.sum().item()]
        ).to(device)
    loss_func = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    optim = torch.optim.Adam(
        model.parameters(), lr=hparams[lr], weight_decay=hparams[wd]
    )
    ## TODO define a LR scheduler
    if lr_scheduler:
        scheduler = ReduceLROnPlateau(
            optim, mode="max", factor=0.1, patience=20, verbose=True
        )
    ...
    for i in range(num_epoch):
        print(f"Epoch {i+1}\n---------------")
        wandb.log({"Epoch": i + 1})
        _, _, train_y, train_preds, train_cts, train_groups = train_epoch(
            model, train_loader, optim, loss_func, batch_size, wandb, device
        )
        if not no_val:
            _, val_auprc, val_y, val_preds, val_cts, val_groups = validate_epoch(
                model, val_loader, loss_func, wandb, device
            )
            if lr_scheduler:
                scheduler.step(val_auprc)  # Update LR based on validation loss
...
def train_epoch(model, train_loader, optim, loss_func, batch_size, wandb, device):
    ...
    (
        ...
        _,
        _,
    ) = get_metrics(all_y, all_preds, all_groups, "training")
...
def validate_epoch(model, val_loader, loss_func, wandb, device):
    ...
    ys, preds, cts, groups = (
        all_y.detach().numpy(),
        torch.sigmoid(all_preds).detach().numpy(),
        all_cts.detach().numpy(),
        all_groups.detach().numpy(),
    )
    ...
    (
        ...
        _,
        _,
    ) = get_metrics(ys, preds, groups, "training")
