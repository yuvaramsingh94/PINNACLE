...
def run_finetune(
    ...
    clf, best_epoch, best_val_auprc, train_ranks, val_ranks = finetune_train_stage(
        X_train,
        y_train,
        random_state,
        groups_train,
    ...
    wandb.log(
        {
            f"train positive proportion celltype": positive_proportion_train[
                "celltype"
            ],
    ...
    positive_proportion_test, auroc_scores, ap_scores = finetune_evaluate(
        celltype_protein_dict,
        clf,
        X_test,
        y_test,
...
def finetune_train_stage(
    ...
    train_ranks, val_ranks = save_torch_train_val_preds(
        best_train_y,
        best_train_preds,
        best_train_groups,
        best_train_cts,
...
def finetune_evaluate(
    ...
    for celltype in celltype_protein_dict:
        ...
        (
            ...
            sorted_preds_test,
            sorted_groups_test,
            positive_proportion_test[celltype],
        ) = get_metrics(y_test, y_test_pred, groups_test, celltype)
        wandb.log(
            {
                f"test AUPRC {celltype}": ap_scores[celltype],
                f"test AUROC {celltype}": auroc_scores[celltype],
                f"test positive proportion {celltype}": positive_proportion_test[
                    celltype
                ],
        ...
        temp = pd.DataFrame(
            {
                "y": sorted_y_test,
                "preds": sorted_preds_test,
                "name": [prot for prot in sorted_groups_test],
            }
        )
        temp["type"] = ["test"] * len(temp)
        test_ranks[celltype] = temp
        temp.to_csv(
            f"{models_output_dir}/{embed_name}_test_preds_{celltype}.csv", index=False
        )  # Save the test predictions
        combined_ranks = pd.concat(
            [train_ranks[celltype], val_ranks[celltype], test_ranks[celltype]], axis=0
        ).sort_values("preds", ascending=False)
        combined_ranks.to_csv(
            f"{models_output_dir}/{embed_name}_all_preds_{celltype}.csv", index=False
        )
...
def main(args, hparams, wandb):
    ...
    positive_proportion_train, positive_proportion_test, auroc_scores, ap_scores = (
        run_finetune(
            embed,
            celltype_dict,
            celltype_protein_dict,
    ...
    output_results_path = os.path.join(
        metrics_output_dir, f"{args.embed}_{args.task_name}_results.csv"
    )
    output_figs_path = os.path.join(
        metrics_output_dir, f"{args.embed}_{args.task_name}_"
    )
