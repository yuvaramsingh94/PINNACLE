...
def path_update_fn(args):
    ...
    args.data_split_path = os.path.join(
        args.result_dir, args.disease, args.data_split_path + "_" + args.disease
    )
    args.positive_proteins_prefix = os.path.join(
        args.result_dir,
        args.disease,
        args.positive_proteins_prefix + "_" + args.disease,
    )
    args.negative_proteins_prefix = os.path.join(
        args.result_dir,
        args.disease,
        args.negative_proteins_prefix + "_" + args.disease,
    )
    args.raw_data_prefix = os.path.join(
        args.result_dir, args.disease, args.raw_data_prefix + "_" + args.disease
    )
    return args
...
def read_args():
    ...
    parser.add_argument(
        "--embeddings_dir", type=str, default="../data/pinnacle_embeds/"
    )
    ...
    parser.add_argument(
        "--result_dir", type=str, default="data/therapeutic_target_task"
    )
    parser.add_argument(
        "--positive_proteins_prefix", type=str, default="positive_proteins"
    )
    parser.add_argument(
        "--negative_proteins_prefix", type=str, default="negative_proteins"
    )
...
def process_and_split_data(
    ...
    pos_prots_group = (
        []
    )  # Protein (needs to be grouped and stay in the same data split)
    neg_prots_group = []
    ...
    for celltype in celltype_dict:
        ...
        pos_indices = np.where(
            np.isin(np.array(celltype_protein_dict[celltype]), np.unique(pos_prots))
        )[0]
        ...
        neg_indices = np.where(
            np.isin(np.array(celltype_protein_dict[celltype]), np.unique(neg_prots))
        )[0]
    ...
    if os.path.exists(data_split_path):  # Generate new splits
        ...
        y_test = {
            celltype: np.concatenate(
                [
                    [
                        1
                        for ind in pos_test_indices
                        if pos_prots_strata[ind] == celltype
                    ],
                    [
                        0
                        for ind in neg_test_indices
                        if neg_prots_strata[ind] == celltype
                    ],
                ]
            )
            for celltype in celltype_dict
        ...
        def get_splits(cv):
            ...
            for i, (pos_train_indices, pos_test_indices) in enumerate(
                cv.split(
                    X=np.arange(len(pos_embed)),
                    groups=pos_prots_group,
                    y=pos_prots_strata,
                )
            ):
                y_test = {
                ...
                if np.all(
                    np.array(list(map(sum, y_test.values())))
                    > TEST_CELLTYPE_POS_NUM_MIN
                ):
                    break

            # Randomly select train/test split for negative examples
            neg_train_indices, neg_test_indices = list(
                iter(
                    cv.split(
                        X=np.arange(len(neg_embed)),
                        groups=neg_prots_group,
                        y=neg_prots_strata,
                    )
                )
            )[np.random.randint(0, n_split)]

            # Ensure that the test set has no overlap with train set
            assert np.all(
                [
                    Counter(pos_prots_group)[prot] == num
                    for prot, num in Counter(
                        np.array(pos_prots_group)[pos_test_indices]
                    ).items()
                ]
            )
            assert np.all(
                [
                    Counter(pos_prots_group)[prot] == num
                    for prot, num in Counter(
                        np.array(pos_prots_group)[pos_train_indices]
                    ).items()
                ]
            )
            ...
            return (
                torch.tensor(pos_train_indices),
                torch.tensor(pos_test_indices),
                torch.tensor(neg_train_indices),
                torch.tensor(neg_test_indices),
                y_test,
            )

        try:
            cv = StratifiedGroupKFold(
                n_splits=n_split, random_state=random_state, shuffle=True
            )  # borrow this CV generator to generate one split of what we want
            (
                pos_train_indices,
                pos_test_indices,
                neg_train_indices,
            ...
            print(
                ...
                Counter(np.array(pos_prots_strata)[pos_train_indices]),
                Counter(np.array(pos_prots_strata)[pos_test_indices]),
            )
            ...
            while count < MAX_RETRY and flag == 0:
                try:
                    ...
                    cv = StratifiedGroupKFold(
                        n_splits=n_split, random_state=new_random_state, shuffle=True
                    )  # borrow this CV generator to generate one split of what we want
                    (
                        pos_train_indices,
                        pos_test_indices,
                    ...
                    print(
                        (
                            "Re-tried successfully with new seed %s,"
                            % str(new_random_state)
                        ),
                        Counter(np.array(pos_prots_strata)[pos_train_indices]),
                        Counter(np.array(pos_prots_strata)[pos_test_indices]),
                    )
            ...
            if flag == 0:
                raise ValueError(
                    f"Could not generate a valid train-test split. Number of positive test samples in some cell types is lower than {TEST_CELLTYPE_POS_NUM_MIN}."
                )

        indices_dict = {
            "pos_train_indices": pos_train_indices.tolist(),
    ...
    X_train = torch.cat(
        [pos_embed[pos_train_indices], neg_embed[neg_train_indices]], dim=0
    )
    groups_train = [pos_prots_group[ind] for ind in pos_train_indices] + [
        neg_prots_group[ind] for ind in neg_train_indices
    ]
    ...
    cts_train = [pos_prots_strata[ind] for ind in pos_train_indices] + [
        neg_prots_strata[ind] for ind in neg_train_indices
    ]
    y_train = np.concatenate(
        [np.ones(len(pos_train_indices)), np.zeros(len(neg_train_indices))]
    )
    ...
    for cat in celltype_dict:
        pos_cat_embs = [
            pos_embed[ind] for ind in pos_test_indices if pos_prots_strata[ind] == cat
        ]
        neg_cat_embs = [
            neg_embed[ind] for ind in neg_test_indices if neg_prots_strata[ind] == cat
        ]

        if len(pos_cat_embs) > 0 and len(neg_cat_embs) > 0:
            X_test[cat] = torch.cat(
                [torch.stack(pos_cat_embs), torch.stack(neg_cat_embs)]
            )
            groups_test[cat] = [
                pos_prots_group[ind]
                for ind in pos_test_indices
                if pos_prots_strata[ind] == cat
            ] + [
                neg_prots_group[ind]
                for ind in neg_test_indices
                if neg_prots_strata[ind] == cat
            ]
            groups_test_pos.extend(
                [
                    pos_prots_group[ind]
                    for ind in pos_test_indices
                    if pos_prots_strata[ind] == cat
            ...
            assert len(set(groups_test[cat]).intersection(set(groups_train))) == 0, set(
                groups_test[cat]
            ).intersection(set(groups_train))
    ...
    for k, v in groups_test.items():
        assert len(set(v).intersection(set(groups_train))) == 0, (
            k,
            set(v).intersection(set(groups_train)),
        )
    ...
    if not os.path.exists(data_split_names_path):  # Generate new splits
        indices_name_dict = {
            "pos_train_names": list(set(groups_train_pos)),
