...
def save_results(
    ...
    auroc_scores: Dict[str, Dict[str, float]],
):
    """
...
def save_torch_train_val_preds(
    best_train_y,
    best_train_preds,
    best_train_groups,
    best_train_cts,
    ...
    for ct in np.unique(best_val_cts):
        ...
        if len(np.unique(val_y_ct)) < 2:
            (
                ...
                sorted_val_y_ct,
                sorted_val_preds_ct,
                sorted_val_groups_ct,
                positive_proportion_val,
            ) = (
                -1,
                -1,
                -1,
            ...
            (
                ...
                sorted_val_groups_ct,
                positive_proportion_val,
            ) = get_metrics(val_y_ct, val_preds_ct, val_groups_ct, "training")
            if len(sorted_val_y_ct) > 0:
                sorted_val_y_ct = sorted_val_y_ct.squeeze(-1)
            if len(sorted_val_preds_ct) > 0:
                sorted_val_preds_ct = sorted_val_preds_ct.squeeze(-1)

        temp = pd.DataFrame(
            {
                "y": sorted_val_y_ct,
                "preds": sorted_val_preds_ct,
                "name": [groups_map_val[prot_ind] for prot_ind in sorted_val_groups_ct],
            }
        )
        temp["type"] = ["val"] * len(temp)
        val_ranks[ct_name] = temp
        temp.to_csv(
            f"{models_output_dir}/{embed_name}_val_preds_{ct_name}.csv", index=False
    ...
    for ct in np.unique(best_train_cts):
        ...
        _, _, _, (sorted_train_y_ct, sorted_train_preds_ct, sorted_train_groups_ct) = (
            precision_recall_at_k(
                train_y_ct, train_preds_ct, k=10, prots=train_groups_ct
            )
        )
        ...
        temp = pd.DataFrame(
            {
                "y": sorted_train_y_ct.squeeze(-1),
                "preds": sorted_train_preds_ct.squeeze(-1),
                "name": [
                    groups_map_train[prot_ind] for prot_ind in sorted_train_groups_ct
                ],
            }
        )
        ...
        temp.to_csv(
            f"{models_output_dir}/{embed_name}_train_preds_{ct_name}.csv", index=False
        )  # Save the validation predictions
...
def precision_recall_at_k(
    y: np.ndarray, preds: np.ndarray, k: int = 10, prots: np.ndarray = None
):
    """
...
def get_metrics(y, y_pred, groups, celltype):
    ...
    recall_10, precision_10, ap_10, (sorted_y, sorted_preds, sorted_groups) = (
        precision_recall_at_k(
            y[celltype], y_pred, k=10, prots=np.array(groups[celltype])
        )
    )
