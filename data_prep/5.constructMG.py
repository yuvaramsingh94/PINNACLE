...
def all_descendants(G, term):
    paths = nx.all_simple_paths(
        G, term, "BTO:0000000"
    )  # BTO:0000000 is the "tissues, cell types and enzyme sources" term
    all_d = set()
    for p in paths:
...
def main():
    ...
    celltype2tissue, unique_tissues = read_tissue_metadata(
        OUTPUT_DIR + "ts_data_tissue.csv", args.annotation, cci.nodes()
    )
    ...
    print(
        "New meta graph (checking that it's the same as the meta graph):\n",
