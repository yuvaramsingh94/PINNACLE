    ...
    for cell, num in Counter(ts_data.obs[groupby].tolist()).items():
        ...
        if num >= num_cells_cutoff:
            ...
            if len(new_data) == 0:
                ...
                new_data = new_data.concatenate(sample, join="inner")
            num_valid_cells += 1
...
def rank_by_stattest(ts_data, ppi, method, groupby, out_f):
    print(
        "Ranking using %s method by `%s`, which has %d unique classes..."
        % (method, groupby, len(ts_data.obs[groupby].unique()))
    )
...
def extract_celltype_ppi(
    input_f, output_f, ppi, lcc=True, max_pval=1, max_number_of_genes=4000
):
    ...
    for i, c in enumerate(sorted(celltype_ppi)):
        c_df = rank_pval[
            [c + "_n", c + "_p"]
        ]  # Extract subset of table corresponding to the cell type c
        c_df_sig = c_df[
            c_df[c + "_p"] <= max_pval
        ]  # Extract subset of genes that satisfy p-value criteria
        c_df_sig = c_df_sig.iloc[
            0:max_number_of_genes
        ]  # Extract subset of genes above a certain threshold
        if len(c_df_sig) == 0:
            continue  # If the list is empty, skip to the next cell type

        c_df_sig_genes = c_df_sig[
            c + "_n"
        ].tolist()  # Create list object of significant genes
        c_subgraph = ppi.subgraph(
            c_df_sig_genes
        )  # Extract the subgraph from the global network corresponding to the significant genes
        if lcc:
            c_lcc = max(nx.connected_components(c_subgraph), key=len)
            print(
                "Size of LCC for %s:" % c,
                len(c_lcc),
                "(%.2f of sig genes in PPI)" % (len(c_lcc) / len(c_subgraph)),
            )
            if len(c_lcc) < 1000:
                continue
...
def aggregate_celltype_ppi_list(
    celltypes, celltype_ppi_list, ppi, cells_per_celltype, out_f
):
    ...
    for i, c in enumerate(celltypes):
        ...
        keep_genes = [
            g for g in gene_counts if gene_counts[g] / num_celltypes_found >= 0.9
        ]
    ...
    calculate_correlation(
        kept_cells_per_celltype,
        frac_nodes,
        "Fraction of nodes kept across >= 0.9 iterations",
        "Number of cells in original cell type",
        "Fraction of nodes kept",
        "figures/frac_nodes_per_cells.pdf",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Constructing cell type specific PPI networks."
    )
    parser.add_argument(
        "-rank", type=bool, help="Perform cell type specific gene ranking."
    )
    parser.add_argument(
        "-annotation",
        type=str,
        default="cell_ontology_class",
        help="Column for cell type annotation.",
    )
    parser.add_argument(
        "-subsample",
        type=bool,
        default=False,
        help="Subsample equal number of cells per cluster.",
    )
    parser.add_argument(
        "-num_cells_cutoff",
        type=int,
        default=100,
        help="Number of cells to subsample (and cutoff for cell types).",
    )
    parser.add_argument(
        "-iterations", type=int, default=10, help="Iterations to subsample."
    )
    parser.add_argument(
        "-rank_pval_filename",
        type=str,
        default=OUTPUT_DIR + "ranked_TabulaSapiens",
        help="Filename (prefix) of ranked genes and their p-values.",
    )
    parser.add_argument(
        "-max_pval",
        type=float,
        default=1,
        help="Maximum p-value threshold for ranked genes.",
    )
    parser.add_argument(
        "-max_num_genes",
        type=int,
        default=4000,
        help="Maximum number of genes to keep (pre-LCC).",
    )
    parser.add_argument(
        "-celltype_ppi_filename",
        type=str,
        default=OUTPUT_DIR + "ppi_TabulaSapiens",
        help="Filename (prefix) of cell type specific PPI.",
    )
    ...
    if args.rank:
        ...
        if args.subsample:
            ...
            for i in range(args.iterations):
                i_data = subsample_cells(
                    ts_data, args.annotation, args.num_cells_cutoff
                )
                if args.iterations > 1:
                    out_f = args.rank_pval_filename + ("_%d" % i)
                else:
                    out_f = args.rank_pval_filename
                i_ts_data, i_rank_pval = rank_by_stattest(
                    i_data, ppi, "wilcoxon", args.annotation, out_f
                )
                print(i_rank_pval)
        else:
            print("Ranking genes per cell type...")
            ts_data, rank_pval = rank_by_stattest(
                ts_data, ppi, "wilcoxon", args.annotation, args.rank_pval_filename
            )
    ...
    # Extract cell type specific PPI
        ...
        if args.subsample:
            ...
            cells_per_celltype = count_cells_per_celltype(
                OUTPUT_DIR + "ts_data_tissue.csv"
            )
            ...
            for i, f in enumerate(glob.glob(args.rank_pval_filename + "*.csv")):
                print(f)
                i_ppi = extract_celltype_ppi(
                    f,
                    "",
                    ppi,
                    lcc=False,
                    max_pval=args.max_pval,
                    max_number_of_genes=args.max_num_genes,
                )
                celltype_ppi_list.append(i_ppi)
                celltypes = celltypes.union(set(list(i_ppi.keys())))

            aggregate_celltype_ppi_list(
                celltypes,
                celltype_ppi_list,
                ppi,
                cells_per_celltype,
                args.celltype_ppi_filename + ("_maxpval=%s.csv" % str(args.max_pval)),
            )
        else:
            extract_celltype_ppi(
                args.rank_pval_filename + ".csv",
                args.celltype_ppi_filename + "_maxpval",
                ppi,
                lcc=True,
                max_pval=args.max_pval,
                max_number_of_genes=args.max_num_genes,
